     ...
     double          *rate)           /* (O) output rate */
{
    ...
    if (discount <= 0.0)
    {
        JpmcdsErrMsg (const_cast < char*>("%s: Bad discount factor %f.\n"),routine,discount) ;
        goto done;
    }
    ...
    if (rateBasis == JPMCDS_DISCOUNT_FACTOR)
    {
        if (startDate == endDate)
        {
            ...
            {
                JpmcdsErrMsg(const_cast < char*>("%s: Start date (%s) equals end date (%s), "
    ...
    if (startDate == endDate)
    {
        JpmcdsErrMsg(const_cast < char*>("%s: Start date %s cannot equal end date.\n"),
                  routine, 
                  JpmcdsFormatDate(startDate));
        goto done;
    }
    ...
    {
        JpmcdsErrMsg (const_cast < char*>("%s: Basis (%ld) < %d.\n"),
    ...
    if (status == FAILURE)
    {
        JpmcdsErrMsg(const_cast < char*>("%s: Failed.\n"), routine);
    }
     ...
     double        *discount)         /* (O) Discount factor */
{
    ...
    if (rateBasis == JPMCDS_DISCOUNT_FACTOR)
    {
       if (rate <= 0.0)
       {
           JpmcdsErrMsg(const_cast < char*>("%s: Bad rate (discount factor) %f\n"),routine,rate);
           goto done;
       }
       *discount = rate;
       return SUCCESS;
    ...
    double *rate )                     /* (O) Output rate */
{
    ...
    if (discount <= 0.0)
    {
        JpmcdsErrMsg(const_cast <char*>("%s: Discount %f <= 0.\n"), routine, discount);
        *rate = 0.0;
        return FAILURE;
    }
    ...
    case JPMCDS_SIMPLE_BASIS:
        ...
        {
            JpmcdsErrMsg(const_cast <char*>("%s: Bad discount:%f or year fraction:%f\n"),
    ...
    case JPMCDS_DISCOUNT_RATE:
        if (IS_ALMOST_ZERO(yearFraction))
        {
            JpmcdsErrMsg(const_cast <char*>("%s: Undefined discount rate\n"),routine);
            *rate = 0.0;
            return FAILURE;
        }
    ...
    case JPMCDS_CONTINUOUS_BASIS:
        if (IS_ALMOST_ZERO(yearFraction))
        {
            JpmcdsErrMsg(const_cast <char*>("%s: Div by zero - yr frac %f.\n"),
    ...
    case JPMCDS_SIMPLE_BASIS:
        {
            ...
            {
                JpmcdsErrMsg(const_cast <char*>("%s: Invalid simple interest rate:%f\n"),
                          routine, rate);
                *discount = 0.0;
                goto done;
            }
    ...
    case JPMCDS_DISCOUNT_RATE:
        ...
        else
        {
            *discount = 1.0 - rate * yearFraction;
            if (*discount <= 0.0)
            {
                JpmcdsErrMsg(const_cast <char*>("%s: Invalid discount rate:%f\n"),
                          routine, rate);
                *discount = 0.0;
                goto done;
            }
    ...
    default:
        {
            ...
            if (tmp <= 0.0 ||
                IS_ALMOST_ZERO(tmp))
            {
                JpmcdsErrMsg(const_cast <char*>("%s: Bad rate: %f.\n"), routine, rate);
                *discount = 0.0;
                goto done;
            }
            else
    ...
    double basis)             /* (I) Compounding basis */
{
    ...
    case JPMCDS_SIMPLE_BASIS:
        ...
        {
            JpmcdsErrMsg(const_cast <char*>("%s: Simple Rate (%f) * Year Fraction (%f) must "
                      "be > -1.0.\n"),
    ...
    case JPMCDS_DISCOUNT_RATE:
        if (rate * yearFraction >= 1.0)
        {
            JpmcdsErrMsg(const_cast <char*>("%s: Discount Rate (%f) * Year Fraction (%f) must "
                      "be < 1.0.\n"),
                      routine, rate, yearFraction);
            goto done;
        }
    ...
    case JPMCDS_DISCOUNT_FACTOR:
        if (rate <= 0.0)
        {
            JpmcdsErrMsg(const_cast <char*>("%s: Discount factor (%f) must be > 0.0.\n"),
                      routine, rate);
            goto done;
        }
        break;
    ...
    default:
        if (rate <= -basis)
        {
            JpmcdsErrMsg(const_cast <char*>("%s: Rate (%f) must be greater than -basis (%f).\n"),
                      routine, rate, -basis);
            goto done;
        }
        break;
