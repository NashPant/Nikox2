...
{
    const char         *name;   /* Upper case name associated with this entry */
    THolidayList *hl;     /* Holiday list */
    //void         *next;
	struct _THoliday* next; /* Next entry in linked list */
} THoliday;
...
static THoliday *holidayFind(const char *name);

/* Delete holiday calendar from cache. */
static void holidayDelete(THoliday *hol);

/* Create holiday calendar. */
static THoliday* JpmcdsNewHoliday(THolidayList *hl, const char *name);
...
(const char* name)
{
    ...
    hl = get_Holiday(name);
    if (hl == NULL) {
        JpmcdsErrMsg("%s: Cannot find calendar.\n", routine);
        goto done;
    }
    status = SUCCESS;
...
//THolidayList* JpmcdsHolidayListFromCache
//(const char *name)    /* (I) Name associated with the holidays */
//{
//    static char   routine[] = "JpmcdsHolidayListFromCache";
//    int           status    = FAILURE;
//
...
(const char         *name,   /* (I) Name to associate holidays with */
...
(THolidayList *hl,   /* (I) Holiday list - takes shallow copy */
 const char         *name  /* (I) Name - takes deep capitalized copy */
)
{
